// app.js — op14: product → description → voice → image
...
async function runLogAndLoad(topic){
  ...
  // === Product picked first ===
  let productLine = appendOverlay("💄 choosing product…","#ffe0f0",true);
  let productElapsed = 0;
  const productTimer = setInterval(()=>{
    productElapsed++;
    productLine.innerText = "💄 choosing product… " + productElapsed + "s";
  },1000);

  // Fetch trend (includes product, desc, image)
  const res = await fetch("https://three23p-backend.onrender.com/api/trend?room="+roomId+"&topic="+topic);
  const trend = await res.json();

  clearInterval(productTimer);
  removeOverlayLine(productLine,`✅ product: ${trend.brand} — ${trend.product}`);
